...
const PerformanceList = () => {
  ...
  useEffect(() => {
    const fetchPerformances = async () => {
      try {
        const response = await axios.get("http://localhost:9999/api/performances", {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        });
        setPerformances(response.data.performances.reverse());
      } catch (error) {
  ...
  return (
    <div className="p-6">
      <div className="text-center mb-4">
        <h3 className="text-2xl font-bold">Performances Summary</h3>
      </div>
      {user.role === "admin" && (
