...
const Detail = () => {
    ...
    const fetchLeave = async () => {
      try {
        const responnse = await axios.get(
          `http://localhost:9999/api/leave/detail/${id}`,
          {
            headers: {
  ...
  const changeStatus = async (id, status) => {
    try {
        const responnse = await axios.put(
          `http://localhost:9999/api/leave/${id}`, {status},
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
  ...
  return (
    <>
      {leave ? (
        <div className="max-w-3xl mx-auto mt-10 bg-[#DDE6ED] p-8 rounded-md shadow-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <img
                src={`http://localhost:9999/${leave.employeeId.userId.profileImage}`}
                ...
                <p className="text-lg font-bold">
                    ...
                    {leave.status === "Pending" ? (
                        <div className="flex space-x-2">
                            <button className="px-2 py-0.5 bg-[#526D82] hover:bg-[#27374D]"
